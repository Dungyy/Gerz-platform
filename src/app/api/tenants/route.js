      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="info-box">
              <p><strong>Your Unit:</strong> ${unit.property.name} - Unit ${unit.unit_number}</p>
              <p><strong>Address:</strong> ${unit.property.address}, ${unit.property.city}, ${unit.property.state}</p>
            </div>
...
export async function POST(request) {
  try {
    const { email, full_name, phone, unit_id, send_sms, invitation_method } = await request.json()

    console.log('👤 Starting tenant invitation for:', email, '- Method:', invitation_method)
    ...
    const { data: managerProfile } = await supabaseAdmin
      .from('profiles')
      .select('organization_id, role, full_name, organization:organizations(name)')
      .eq('id', manager.id)
      .single()
    ...
    if (phone && send_sms) {
      try {
        const smsMessage = invitation_method === 'magic_link'
          ? `Welcome to ${unit.property.name}! Check your email (${email}) for a secure login link to access your tenant portal. - Gerz`
          : `Welcome to ${unit.property.name}! Check your email (${email}) to set your password and access your tenant portal. - Gerz`
...
export async function GET(request) {
  try {
    ...
    if (profile.role !== 'owner' && profile.role !== 'manager') {
      return NextResponse.json({ 
        error: 'Insufficient permissions - Only managers/owners can view tenants' 
      }, { status: 403 })
    }

    console.log('✅ Authorization passed - fetching tenants for org:', profile.organization_id)
