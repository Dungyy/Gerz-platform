...
export async function POST(request) {
  try {
    const { user_id, message_type, custom_message, request_data } =
      await request.json();
    ...
    if (preferences) {
      if (message_type === "new_request" && !preferences.sms_new_request) {
        return NextResponse.json({
          success: false,
          message: "SMS disabled for this type",
        });
      }
...
function generateMessage(type, data, userName) {
  ...
  switch (type) {
    case "status_update":
      return `Hi ${userName}, your maintenance request "${
        data.title
      }" status changed to ${data.status.replace(
        "_",
        " "
      )}. View: ${appUrl}/dashboard/requests/${data.id}`;
    ...
    default:
      return (
        data.message ||
        "You have a new notification. Check the Dingy.app app for details."
      );
  }
}
